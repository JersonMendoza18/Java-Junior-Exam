...
async function showStart() {
  view = "start";
  try { sessionStorage.removeItem("sid"); } catch { /* storage unavailable */ }
  let meta;
...
function startQuiz(st) {
  view = "quiz";
  try { sessionStorage.setItem("sid", st.id); } catch { /* storage unavailable */ }
...
(async function init() {
  let sid = null;
  try { sid = sessionStorage.getItem("sid"); } catch { /* storage unavailable */ }
  if (sid) {
    try {
